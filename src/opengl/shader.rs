...
pub mod render_options {
    ...
    unsafe impl<T: RemoveFront + Copy> RenderOptions for T
    where
        T::Front: RenderOptions,
        T::Remaining: RenderOptions,
    {
        unsafe fn with_options<F: FnOnce()>(self, gl: &Gl, clo: F) {
            let (front, remaining) = self.remove_front();
            front.with_options(gl, || remaining.with_options(gl, clo));
        }
    }
    ...
    macro_rules! func {
        ($t:ident, $($f:ident),*) => {
            $(
                pub const $f: $t = $t { val: gl::$f };
            )*
        };
    }

    func!(DepthFunc, ALWAYS, NEVER, NOTEQUAL, EQUAL, GREATER, LESS, GEQUAL, LEQUAL);
    func!(StencilOp, KEEP, INVERT, ZERO, REPLACE, INCR, DECR, INCR_WRAP, DECR_WRAP);
    ...
    /// The stencil test is a bit more complicated than the depth test. Here
    /// is how it is executed:
    /// * A fragment is generated by the fragment shader. The stencil value for this fragment
    /// is `stecil_val`. Fragments discarded by the fragment shader will not effect the stecil buffer.
    /// * The value is compared with the value already in the stecil buffer. For convienience, call
    /// this value `D`. The stecil test passes if `stecil_val & mask func D & mask`. If there is no stencil buffer
    /// bound, stenciling will behave as if it is disabled - the stecil buffer will not be updated
    /// and no fragments will be discarded. If this test fails, then the value in the stecil buffer
    /// will be updated based on `fail_op`.
    /// * If the previous test passes, then the the depth test occurs. If the depth test fails, then
    /// the value in the stecil buffer will be updated according to `depth_fail_op`, otherwise, the
    /// `depth_pass_op` will be used. The color buffer(s) are only updated if both tests pass. If
    /// depth testing is disabled, then `depth_pass_op` will always be used if the stencil test fails.
    #[derive(Clone, Copy)]
    pub struct StencilTest {
        pub enabled: bool,
        /// the stencil test passes if the current fragment value is `pass` the
        /// value it would replace, so if a shader wants to draw a fragment with
        /// stencil value 10 when the existing stencil value is 22, and `pass` is
        /// set to `GREATER`, then the test will fail
        pub func: DepthFunc,
    ...
    /// It is possible for the options for stenciling to be different depending
    /// on whether the triangle being draw is front or back face. Primitives other
    /// than triangles will use the options for the front face.
    #[derive(Clone, Copy)]
    pub struct StencilFaces {
    ...
    unsafe impl RenderOptions for StencilTest {
        unsafe fn with_options<F: FnOnce()>(self, gl: &Gl, clo: F) {
            if self.enabled {
                ...
                gl.StencilFuncSeparate(
                    gl::FRONT_AND_BACK,
                    self.func.val,
                    self.stencil_val as i32,
                    self.mask as u32,
                );
                gl.StencilOpSeparate(
                    gl::FRONT_AND_BACK,
                    self.stencil_fail_op.val,
                    self.depth_fail_op.val,
                    self.depth_pass_op.val,
                );
                clo();
            } else {
                clo();
    ...
    unsafe impl RenderOptions for StencilFaces {
        unsafe fn with_options<F: FnOnce()>(self, gl: &Gl, clo: F) {
            if self.front.enabled | self.back.enabled {
                gl.Enable(gl::STENCIL_TEST);
                if self.front.enabled {
                    gl.StencilFuncSeparate(
                        gl::FRONT,
                        self.front.func.val,
                        self.front.stencil_val as i32,
                        self.front.mask as u32,
                    );
                    gl.StencilOpSeparate(
                        gl::FRONT,
                        self.front.stencil_fail_op.val,
                        self.front.depth_fail_op.val,
                        self.front.depth_pass_op.val,
                    );
                } else {
                ...
                if self.back.enabled {
                    gl.StencilFuncSeparate(
                        gl::BACK,
                        self.back.func.val,
                        self.back.stencil_val as i32,
                        self.back.mask as u32,
                    );
                    gl.StencilOpSeparate(
                        gl::BACK,
                        self.back.stencil_fail_op.val,
                        self.back.depth_fail_op.val,
                        self.back.depth_pass_op.val,
                    );
                } else {
                    gl.StencilFuncSeparate(gl::FRONT, gl::ALWAYS, 0, 0);
                    gl.StencilOpSeparate(gl::FRONT, gl::KEEP, gl::KEEP, gl::KEEP);
                ...
                clo();
                // stenciling is never enabled without setting the stencil op
    ...
    /// The main use of this option is for face culling, but it can also be used
    /// by stenciling operation. By default, the front face is CCW.
    #[derive(Clone, Copy)]
    pub struct FrontFace {
        pub front_is_ccw: bool,
    }

    unsafe impl RenderOptions for FrontFace {
        unsafe fn with_options<F: FnOnce()>(self, gl: &Gl, clo: F) {
            if !self.front_is_ccw {
                gl.FrontFace(gl::CW);
                clo();
                gl.FrontFace(gl::CCW);
            } else {
    ...
    /// As an optimization, the back face of triangles can be discarded.
    /// Which face is the front or back is determined by winding order, with
    /// the mode used to specify this determined by the argument to `FrontFace`
    /// If no `FrontFace` option is passed to a draw call, then the front face will
    /// be CCW.
    ///
    /// Interestingly, opengl allows culling both the front and back faces. This effectively
    /// disables rendering triangles, but points and lines will still be rendered since they
    /// do not have faces.
    ///
    /// If both front and backface culling are disabled by `CullFace`, then face culling will be
    /// disabled.
    #[derive(Clone, Copy)]
    pub struct CullFace {
    ...
    unsafe impl RenderOptions for CullFace {
        unsafe fn with_options<F: FnOnce()>(self, gl: &Gl, clo: F) {
            if self.cull_front_face | self.cull_back_face {
                gl.Enable(gl::CULL_FACE);
                if self.cull_front_face && self.cull_back_face {
                    gl.CullFace(gl::FRONT_AND_BACK);
                    clo();
                } else if self.cull_front_face {
    ...
    #[derive(Clone, Copy)]
    pub struct MaskDepth {
        pub replace_depth: bool,
    }
...
pub mod vec {
    ...
    macro_rules! map_element_for {
        ($e:ty, $v:ty) => {
            unsafe impl<T: VecType> MapFor<T, $v> for $e {
                type L_Out = Vec1;
    ...
    map_element_for!(X, Vec1);
    map_element_for!(X, Vec2);
    map_element_for!(X, Vec3);
    map_element_for!(X, Vec4);
    map_element_for!(Y, Vec2);
    map_element_for!(Y, Vec3);
    map_element_for!(Y, Vec4);
    map_element_for!(Z, Vec3);
    map_element_for!(Z, Vec4);
    map_element_for!(W, Vec4);
